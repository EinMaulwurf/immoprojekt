...
new_data_brennpunktFALSE <- data.frame(jahr = (2007:2020)-2007,
           ist_brennpunkt = FALSE) %>%
   mutate(mietekalt_m2_log = predict(lm_miete_brennpunkt, .),
          mietekalt_m2 = exp(mietekalt_m2_log))
new_data_brennpunktFALSE

new_data_brennpunktTRUE <- data.frame(jahr = (2007:2020)-2007,
           ist_brennpunkt = TRUE) %>%
   mutate(mietekalt_m2_log = predict(lm_miete_brennpunkt, .),
          mietekalt_m2 = exp(mietekalt_m2_log))
new_data_brennpunktTRUE

data.frame(jahr = new_data_brennpunktFALSE$jahr + 2007,
           nichtBrennpunktgebiet = new_data_brennpunktFALSE$mietekalt_m2,
           Brennpunktgebiet = new_data_brennpunktTRUE$mietekalt_m2) %>%
  pivot_longer(cols = -jahr, names_to = "ist_brennpunkt", values_to = "miete_m2") %>%
  ggplot(aes(x = jahr, y = miete_m2, color = ist_brennpunkt))+
  geom_line()+
  scale_x_continuous(breaks = seq(from = 2007, to = 2020, by = 2))+
  scale_color_manual(labels = c("Brennpunkt", "nicht-Brennpunkt"), values = c("blue", "red")) +
  theme_bw()+
  theme(legend.position = c(0.22, 0.8))+
  labs(x = "Jahr", y = "Kaltmiete pro m²",
       color = element_blank())

ggsave("./plots/plot_regression_brennpunkte_modell.png", width = 5, height = 3)
```
...
```{r}
test_data <- data.frame(jahr = c(0,0,1,1),
           ist_brennpunkt = c(TRUE, FALSE, TRUE, FALSE))

predict(lm_miete_brennpunkt, test_data) %>% exp()
```
...
data_social %>%
  ...
  filter(mietekalt_m2 < quantile(mietekalt_m2, 0.99), mietekalt_m2 > quantile(mietekalt_m2, 0.01)) %>%
  filter(jahr == 2007) %>%
  group_by(ist_brennpunkt) %>%
  summarise(mean(mietbelastung),
            mean(mietekalt_m2))
```
