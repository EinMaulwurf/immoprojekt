...
new_data_miete_brennpunktFALSE <- data.frame(jahr = (2007:2020)-2007,
           ist_brennpunkt = FALSE) %>%
   mutate(mietekalt_m2 = exp(predict(lm_miete_brennpunkt, .)),
          mietbelastung = exp(predict(lm_mietbelastung_brennpunkt, .)))
#new_data_miete_brennpunktFALSE

new_data_miete_brennpunktTRUE <- data.frame(jahr = (2007:2020)-2007,
           ist_brennpunkt = TRUE) %>%
   mutate(mietekalt_m2 = exp(predict(lm_miete_brennpunkt, .)),
          mietbelastung = exp(predict(lm_mietbelastung_brennpunkt, .)))
#new_data_miete_brennpunktTRUE
...
rbind(new_data_miete_brennpunktFALSE, new_data_miete_brennpunktTRUE) %>%
  mutate(jahr = jahr + 2007,
         jahr = lubridate::ymd(paste(jahr, 06, 01, sep = "-"))) %>%
