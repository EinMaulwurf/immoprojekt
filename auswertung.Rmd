...
Betrachtet man die Regression zur Mietbelastung, so erkennt man, dass auch diese über die Zeit angestiegen ist. Die abhängige Variable wurde in diesem Modell nicht log-transformiert, wodurch sich die Koeffizienten der Regression direkt interpretieren lassen. Die Konstante zeigt, dass die Mietbelastung im Jahr 2007 bei knapp 14\% liegt. Pro Jahr steigt dieser Wert dann im Durchschnitt um 1.3pp. Sowohl die Konstante als auch der Effekt pro Jahr sind statistisch signifikant.
...
Ein Problem ist jedoch Heteroskedastizität. So nennt man das Phänomen von nicht stabilen Varianzen. Für diesen Fall konkret sehen wir, dass die Varianz über die Jahre steigt, die Preise für Wohnungen streuen also immer weiter. Mathematisch bringt das Probleme mit der Robustheit der Schätzung, insbesondere kann dies zu Verzerrungne in den Standardfehlern führen. Diese werden wiederum für den t-Test und die Bestimmung der statistischen Signifikanz benötigt.

Um dieses Problem zu umgehen, berechnen wir im folgenden robuste Standardfehler und Vergleichen die Ergebnisse mit der ursprünglichen Regression. Dafür wird eine "heteroskedasticity-consistent" (HC) Kovarianzmatrix geschätzt.
...
```{r}
data_lq <- data_social_sf %>%
  select(r1_id, jahr, arbeitslosenquote, anzahl_haushalte) %>%
  group_by(jahr) %>%
  mutate(LQ_arbeitslosenquote = arbeitslosenquote / weighted.mean(arbeitslosenquote, anzahl_haushalte, na.rm = TRUE))
...
p2 <- test %>%
  filter(jahr == 2005) %>%
  ggplot()+
  geom_sf(aes(fill = LQ_arbeitslosenquote))+
  scale_fill_viridis_c()+
  theme_void()+
  theme(legend.position = "top")
